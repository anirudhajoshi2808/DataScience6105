...
s = filter(df, Grad.Rate >= 80 & Grad.Rate <= 98)
a = arrange(s, desc(Grad.Rate))
select(a, X, Grad.Rate)
...
# DF
df <- college[270:299,]
dfq <- select(df, Private, Outstate)
dfq
dfr <- group_by(dfq, Private) %>% summarise(Max = max(Outstate),Min = min(Outstate)) %>%arrange(Private)
dfr

# College
dfn <- college
dfa <- select(dfn, Private, Outstate)
dfa
dfb <- group_by(dfa, Private) %>% summarise(Max = max(Outstate),Min = min(Outstate)) %>%arrange(Private)
dfb
